...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        # get the id of a product
        test_product = self._create_products(1)[0]
        test_product.name = "football"
        test_product.description = "football is a good sport"
        ...
        invalid_id = test_product.id + 1
        response = self.client.put(f"{BASE_URL}/{invalid_id}", json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        data = response.get_json()
        self.assertEqual(data["message"], f"404 Not Found: Product with id '{invalid_id}' was not found.")
    ...
    def test_query_by_price(self):
        ...
        price_products = [product for product in products if product.price == price]
        price_count = len(price_products)

        response = self.client.get(
            BASE_URL, query_string=f"price={price}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
